...
SELECT o.name, o.display_name, o.id, count(case when t.eliminated = 0 then 1 else null end) as players_remaining, sum(round1), sum(round2), sum(round3), sum(round4), sum(round5), sum(round6), (sum(round1) + sum(round2) + sum(round3) + sum(round4) + sum(round5) + sum(round6)) as 'Total' FROM mm.owner o, mm.player p, mm.team t WHERE p.owner_id = o.id AND p.team_id = t.id group by o.id order by Total desc;;
...
SELECT name, display_name, ROW_NUMBER() OVER (ORDER BY RAND()) as draft_position FROM mm.owner ;;
