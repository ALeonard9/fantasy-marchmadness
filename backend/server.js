...
app.get('/playerboard', async(req, res) => {
  var response = await mysql_lib.mysql_query('Playerboard', 'SELECT o.display_name, o.name, o.id, p.full_name, p.espn_id, p.scoring_average, p.projected_score, p.position, p.class, p.drafted_round, p.drafted_position, p.jersey, p.round1, p.round2, p.round3, p.round4, p.round5, p.round6, (round1 + round2 + round3 + round4 + round5 + round6) as total, t.school, t.mascot, t.id as team_id, t.seed, t.region, t. eliminated FROM mm.team t, mm.player p LEFT JOIN mm.owner o ON p.owner_id = o.id WHERE  t.id = p.team_id ORDER BY seed, school, scoring_average desc;');
  res.send(JSON.stringify(response));
});
app.get('/draft/results', async(req, res) => {
  var response = await mysql_lib.mysql_query('Draft results', 'SELECT o.display_name, o.name, o.id, p.full_name, p.id as player_id, p.scoring_average, p.projected_score, p.drafted_round, p.drafted_position, p.jersey, t.school, t.mascot, t.seed, t.region, t. eliminated, (round1 + round2 + round3 + round4 + round5 + round6) as total FROM mm.team t, mm.player p LEFT JOIN mm.owner o ON p.owner_id = o.id WHERE  t.id = p.team_id AND p.owner_id is not null ORDER BY drafted_position, projected_score;');
  res.send(JSON.stringify(response));
});
...
app.get('/update_player_draft_position', async(req, res) => {
  var response = await scraper.update_player_draft_position();
  res.send(JSON.stringify(response));
});
app.get('/update_player_draft_position/:id', async(req, res) => {
  var response = await scraper.update_player_draft_position(req.params.id);
  res.send(JSON.stringify(response));
});
